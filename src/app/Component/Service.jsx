...
export default function Services() {
  return (
    <section className="px-4 md:px-20 lg:px-40 py-16 bg-white dark:bg-slate-900/50" id="services">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div key={index} className="group p-8 bg-background-light dark:bg-background-dark rounded-xl border border-[#ced8e8] dark:border-slate-800 hover:border-primary transition-all hover:shadow-xl">
              <div className="mb-6 inline-flex items-center justify-center size-14 bg-primary/10 rounded-lg text-primary">
                {service.icon}
              </div>
              <h3 className="text-xl font-bold mb-3">{service.title}</h3>
              <p className="text-slate-custom dark:text-slate-400 leading-relaxed mb-6">{service.desc}</p>
